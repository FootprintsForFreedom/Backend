    ...
    func testSuccessfulListRepositoriesWithUnverifiedModels() async throws {
        ...
        try app
            ...
            .expect(AppApi.Page<Tag.Detail.List>.self) { content in
                ...
                XCTAssertEqual(content.items.map(\.id).uniqued().count, unverifiedTagCount)
    ...
    func testListUnverifiedDetailsForRepository() async throws {
        ...
        try app
            ...
            .expect(AppApi.Page<Tag.Repository.ListUnverified>.self) { content in
                ...
                XCTAssertEqual(content.items.map(\.detailId).uniqued().count, unverifiedTagForRepositoryCount)
