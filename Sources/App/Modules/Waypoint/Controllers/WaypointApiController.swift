    ...
    func listWrapper(_ req: Request) async throws -> Waypoint.Detail.ListWrapper {
        ...
        if decodedLocation.latitude == nil || decodedLocation.longitude == nil {
            if let userIp = req.peerAddress?.ipAddress,
               case .value(let result) = req.mmdb.search(address: userIp),
               case .map(let map) = result,
               case .map(let locationMap) = map["location"],
               case .double(let latitude) = locationMap["latitude"],
               case .double(let longitude) = locationMap["longitude"]
            {
