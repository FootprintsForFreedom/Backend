...
extension Array where Element: Equatable {
    ...
    func difference(from other: [Element]) -> (equal: [Element], deleted: [Element], inserted: [Element]) {
        let combinations = map { firstElement in (firstElement, other.first { $0 == firstElement }) }
        let equal = combinations.filter { $0.1 != nil }.map(\.0)
        let deleted = combinations.filter { $0.1 == nil }.map(\.0)
